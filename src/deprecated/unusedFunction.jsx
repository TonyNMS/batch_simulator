...
const PowerTrainConfigFileDropZone =()=>{ 
    ...
    const onDrop = useCallback( 
        (droppedFile)=>{ const file = droppedFile[0] 
    ...
    const {getRootProps, getInputProps, isDragActive} = useDropzone( 
        { onDrop, accept: {'application/octet-stream':['.csv']} } ) 
                ...
                csvFile, { 
                    ...
                    complete: ({ data, errors }) => 
                        { if (errors && errors.length) 
                            { console.warn("Papa errors:", errors); } resolve(data); }, 
        ...
        while (temp_parsed_csv[engineCounter][0] !='' && temp_parsed_csv[engineCounter][0] != null){ 
            ...
            if (temp_parsed_csv[engineCounter][6] === "Diesel"){ 
                const engineObj ={ 
                    ...
                    temp_die_eng_container.push(engineObj) } engineCounter++ } 
    ...
    const extractAllBattery = async (csvFile)=>{ 
            ...
            let temp_battery_container = [] /*Read the csv */ 
            const temp_parsed_csv = await parseRawCSV(csvFile) /*Extract Battery Objects */ 
            console.log(temp_parsed_csv) 
            let batteryCounter = 1 
            while (
                temp_parsed_csv[batteryCounter][9] != "" && temp_parsed_csv[batteryCounter][9] != null ){ 
                    const temp_battery_obj = { 
                        "battery_name" : temp_parsed_csv[batteryCounter][9], 
                        ...
                        "battery_charge_rate": parseFloat(temp_parsed_csv[batteryCounter][16]) * parseFloat(temp_parsed_csv[batteryCounter][15]), "battery_max_charge_power" : parseFloat(temp_parsed_csv[batteryCounter][14]) * parseFloat(temp_parsed_csv[batteryCounter][15]) } 
                        temp_battery_container.push(temp_battery_obj) 
                        batteryCounter++ }
                        /*Send the container to update the state */ setBatteryList(temp_battery_container) /*Send the container as a globle context */ 
