model AZEAT_Validation
  ...
  // External Model Repository for Local AZEAT_Validation Use
  /*Master Controller*/
  ...
  model MasterControllerSingleBattery
    ...
    model MethanolBatteryDieselDebug
      ...
      parameter Modelica.Units.SI.Power P_idling_alt_gen "Alternative Fuel Generator Output " annotation(
        Dialog(group = "All generators"));
      parameter Modelica.Units.SI.Power P_ref_FC_max_rate = 0;
      ...
      //Real powerBandSize;
      Boolean chargingStatus = false "Battery Charging Required ?";
...
// no need to run the battery
              P_fuelCellGen = 0;
              IceGenChargingRate = if (SOC_1 < 0.9 and P_rated_battery > 1) then DieselGeneratorCount*dieselOptimLowerBound - P_net else 0;
...
/*Leaf Node Special 2.5 */
              ...
              P_battery_1 = 0;
              P_fuelCellGen = 0;
              IceGenChargingRate = if (SOC_1 < 0.9 and P_rated_battery > 1) then P_idling_diesel_gen - P_net else 0;
              nodeTracker = 2.5;
            elseif (P_net > DieselGeneratorCount*dieselOptimUpperBound) then
              if (SOC_1 > 0.2 and P_rated_battery > 1) then
...
// When demand is  neither  larger than rated power not within higher bound and  rated power
              ...
              elseif (SOC_1 < 0.2 or P_rated_battery < 1) then
/*Leaf Node 8*/
//Prioritise Diesel, Diesel Charging , Demand Bigger than BSFC  Higher Bound. Battery NON - Exist.
                P_dieselGen = if (P_rated_alt_gen > 0) then DieselGeneratorCount*dieselOptimUpperBound else P_net;
...
/*Leaf Node 26 */
// Prioritise Alt Fuel, Diesel Charging , Demand Smaller than BSFC Lower Bound  == Load Balancing then Charging  ==
              P_altFuelGen = if (SOC_1 > 0 and P_rated_battery > 1) then AltFuelGeneratorCount*altFuelOptimLowerBound else P_net;
              P_dieselGen = 0;
              P_battery_1 = 0;
              P_fuelCellGen = 0;
              IceGenChargingRate = if (SOC_1 > 0 and P_rated_battery > 1) then AltFuelGeneratorCount*altFuelOptimLowerBound - P_net else 0;
              nodeTracker = 26;
// elseif the demand is smaller the idling power
            elseif (P_net < P_idling_alt_gen or P_net == P_idling_diesel_gen) then
              P_altFuelGen = P_idling_alt_gen;
              ...
              P_battery_1 = 0;
              P_fuelCellGen = 0;
              IceGenChargingRate = if (SOC_1 > 0 and P_rated_battery > 1) then P_idling_alt_gen - P_net else 0;
              nodeTracker = 26.5;
            elseif (P_net > AltFuelGeneratorCount*altFuelOptimUpperBound) then
              if (SOC_1 > 0.2 and P_rated_battery > 1) then
                if (P_net > P_rated_diesel_gen) then
        ...
        if icePriorityAssignment == 0 then
          if iceChargingProrityAssignement == 0 then
            ...
            elseif (P_net < DieselGeneratorCount*dieselOptimLowerBound) then
              ...
              nodeTracker = 2.9;
            elseif (P_net > DieselGeneratorCount*dieselOptimUpperBound) then
              if (SOC_1 > 0.2 and P_rated_battery > 0) then
                if (P_net > P_rated_diesel_gen) then
                  if (P_net - DieselGeneratorCount*dieselOptimUpperBound < P_rated_battery) then
/*Leaf Node 3 */
// Prioritise  Diesel, Diesel Charging, Demand Bigger than BSFC Higher Bound, Battery Exist, Demand Also Higher than Max Rated Power, Battery Can Cover the Gap
...
// When demand is  neither  larger than rated power not within higher bound and  rated power
              ...
              elseif (SOC_1 < 0.2 or P_rated_battery == 0) then
/*Leaf Node 7*/
//Prioritise Diesel, Diesel Charging , Demand Bigger than BSFC  Higher Bound. Battery NON - Exist.
                P_dieselGen = if (P_rated_alt_gen > 0) then DieselGeneratorCount*dieselOptimUpperBound else P_rated_diesel_gen;
                P_battery_1 = 0;
...
// priorityAssignement end
    ...
    model MethanolBatteryDieselControllerCore
      ...
      parameter Modelica.Units.SI.Power P_charging_min = -P_charging_max "Maximum discharge rate" annotation(
        Dialog(group = "Battery"));
      parameter Modelica.Units.SI.Power P_rated_diesel_gen = 800000 "Generator max output" annotation(
        Dialog(group = "Diesel generator"));
      parameter Modelica.Units.SI.Power P_idling_diesel_gen = 100000 "Diesel Generator  Idle Output " annotation(
        Dialog(group = "Diesel generator"));
      parameter Modelica.Units.SI.Power P_rated_alt_gen = 800000"Alternative Fuel Generator max_output " annotation(
        Dialog(group = "All generators"));
      parameter Modelica.Units.SI.Power P_idling_alt_gen = 100000 "Alternative Fuel Generator Output " annotation(
        Dialog(group = "All generators"));
      parameter Modelica.Units.SI.Power P_ref_FC_max_rate = 10000;
      parameter Modelica.Units.SI.Power P_rated_combined_gen = 0 "Combined avalible generator output" annotation(
        Dialog(group = "All generators"));
      parameter Modelica.Units.SI.Power P_rated_battery = 15000 "Max Output of  Battery Stack ";
      parameter Modelica.Units.SI.Power DieselGenChargingRate = 1000 "Diesel charging rate to battery, measured in Watt" annotation(
        Dialog(tab = "ICE Charging Configuration"));
      ...
      parameter Real IceTankChargingLimit = 0.3 "ICE Fuel Tank minimal SOC limit to charge the battery" annotation(
        Dialog(tab = "ICE Charging Configuration"));
      Modelica.Units.SI.Power IceGenChargingRate "Current ICE charging rate assigned with this simulation " annotation(
        Dialog(tab = "ICE Charging Configuration"));
      // Real  P_iceForCharginRate;
      // Modelica.Blocks.Interfaces.RealOutput P_iceCharging  annotation(
      //Placement(transformation(origin = {110, 90}, extent = {{-10, -10}, {10, 10}}), iconTransformation(origin = {110, 90}, extent = {{-10, -10}, {10, 10}})));
      Modelica.Blocks.Interfaces.RealInput dieselOptimUpperBound annotation(
        Placement(transformation(origin = {-110, -10}, extent = {{-10, -10}, {10, 10}}), iconTransformation(origin = {-50, -110}, extent = {{-10, -10}, {10, 10}}, rotation = 90)));
...
// Prioritise  Diesel, Alt Fuel Charging , Demand within the BSFC Sweet Spot Zone
              P_dieselGen = P_net;
// diesel generator handels the deamnd
              P_altFuelGen = if (P_rated_alt_gen > 0) then if (SOC_1 < 0.9 and P_battery_1 > 0) then AlthFuelGenChargingRate else 0 else 0;
// if there is a  altFuelGen and the battery need s charging then  charge the battery
              P_battery_1 = 0;
// no battery need s to be invovled
              P_fuelCellGen = 0;
...
//if there is alt gen then die gen runs at upper bound or else runs at max diesel gen
                P_battery_1 = 0;
// no battery  power
                P_altFuelGen = if (P_rated_alt_gen > 0) then if (P_rated_battery == 0) then P_net - P_dieselGen else (P_net - P_dieselGen) + AlthFuelGenChargingRate else 0;
// if there ius alt gen then check if ther eis battery, if there is battery run at  unmet + RatedChargerPower  else
                IceGenChargingRate = if (P_rated_alt_gen > 0) then if (P_rated_battery == 0) then 0 else (AlthFuelGenChargingRate) else 0;
// if there is alt gen , then checl if there is a attery, if there is  then  charging at  AlthFuelGenChargingRate else  0
                P_fuelCellGen = 0;
...
// Prioritise Alt Fuel, Diesel Charging , Demand within the BSFC Sweet Spot Zone
              P_altFuelGen = P_net;
              P_dieselGen = if (P_rated_diesel_gen > 0) then if (SOC_1 < 0.9 and P_battery_1 > 0) then DieselGenChargingRate else 0 else 0;
              P_battery_1 = 0;
              P_fuelCellGen = 0;
              IceGenChargingRate = P_dieselGen;
              nodeTracker = 17;
...
//Prioritise Alt Fuel, Diesel Charging , Demand Bigger than BSFC  Higher Bound. Battery NON - Exist.
                P_altFuelGen = if (P_rated_diesel_gen > 0) then AltFuelGeneratorCount*altFuelOptimUpperBound else P_rated_alt_gen;
                P_battery_1 = 0;
                P_dieselGen = if (P_rated_diesel_gen > 0) then if (P_rated_battery == 0) then 0 else AlthFuelGenChargingRate else 0;
                IceGenChargingRate = P_altFuelGen;
                P_fuelCellGen = 0;
                nodeTracker = 24;
              else
...
// Prioritise Alt Fuel, Alt Fuel Charging , Demand within the BSFC Sweet Spot Zone
              ...
              IceGenChargingRate = 0;
              nodeTracker = 25;
            elseif (P_net < AltFuelGeneratorCount*altFuelOptimLowerBound) then
/*Leaf Node 26 */
// Prioritise Alt Fuel, Diesel Charging , Demand Smaller than BSFC Lower Bound  == Load Balancing then Charging  ==
              P_altFuelGen = if (SOC_1 > 0 and P_rated_battery > 0) then AltFuelGeneratorCount*altFuelOptimLowerBound else P_net;
              P_dieselGen = 0;
              P_battery_1 = 0;
              P_fuelCellGen = 0;
              IceGenChargingRate = if (SOC_1 > 0 and P_rated_battery > 0) then AltFuelGeneratorCount*altFuelOptimLowerBound - P_net else 0;
              nodeTracker = 26;
            elseif (P_net > AltFuelGeneratorCount*altFuelOptimUpperBound) then
              if (SOC_1 > 0.2 and P_rated_battery > 0) then
                if (P_net > P_rated_diesel_gen) then
...
/*
    ...
    if (SOC_1 > 0.2 and P_rated_battery > 0) then
        if (P_net > P_rated_diesel_gen) then
          if (P_net - DieselGeneratorCount*dieselOptimUpperBound < P_rated_battery) then
              ...
              IceGenChargingRate = 0;
              P_fuelCellGen = 0;
          elseif (P_net - DieselGeneratorCount*dieselOptimUpperBound > P_rated_battery) then
              if (P_rated_alt_gen > 0) then
                P_dieselGen = DieselGeneratorCount*dieselOptimUpperBound;
                P_battery_1 = P_rated_battery*smooth_soc_min;
                P_altFuelGen = P_net - DieselGeneratorCount*dieselOptimUpperBound - P_rated_battery;
                IceGenChargingRate = 0;
                P_fuelCellGen = 0;
              elseif (P_rated_alt_gen == 0) then
                P_dieselGen = P_rated_diesel_gen;
                P_battery_1 = P_rated_battery*smooth_soc_min;
                P_altFuelGen = 0;
                IceGenChargingRate = 0;
        ...
        elseif (P_net < P_rated_diesel_gen and P_net > DieselGeneratorCount*dieselOptimUpperBound) then
          if (P_rated_battery > P_net - DieselGeneratorCount*dieselOptimUpperBound) then
            P_dieselGen = DieselGeneratorCount*dieselOptimUpperBound;
            P_battery_1 = P_net - DieselGeneratorCount*dieselOptimUpperBound;
            P_altFuelGen = 0;
            IceGenChargingRate = 0;
            P_fuelCellGen = 0;
          elseif (P_rated_battery < P_net - DieselGeneratorCount*dieselOptimUpperBound) then
            P_dieselGen = DieselGeneratorCount*dieselOptimUpperBound;
    ...
    */
      annotation(
        Icon(graphics = {Rectangle(fillPattern = FillPattern.Solid, extent = {{-80, 80}, {80, -80}})}));
...
// Find how many units are required to run
  ...
  package Fuel
    ...
    package compressedhydrogen
      extends AZEAT_Validation.Fuel.MethanolFuel(LHV = 120e6, rho = 26, rho_liq = 26, MolarMass = 1.00899e-3, Delta_h_evap = 2.01588*1e-3);
  ...
  model Generator
    ...
    model Generator_Base
      ...
      connector FluidPort "Fluid port for low fidelity models in microgrid components"
        replaceable package Medium = AZEAT_Validation.Fuel.Hydrogen constrainedby AZEAT_Validation.Fuel.SimpleFuel "Transported medium";
      ...
      replaceable package Fuel = AZEAT_Validation.Fuel.Diesel constrainedby AZEAT_Validation.Fuel.SimpleFuel "Fuel type" annotation(
         choicesAllMatching,
      ...
      FluidPort co2_outlet(redeclare package Medium = AZEAT_Validation.Fuel.CarbonDioxide, m_flow = -m_flow_CO2) if use_external_co2_sink annotation(
        Placement(transformation(extent = {{-10.0, -110.0}, {10.0, -90.0}}, rotation = 0.0, origin = {0.0, 0.0})));
    protected
...
// for mathematical stability , if constant efficiency is used this variable is not used in computation but it should be assigned with something for numerical balance
  ...
  model IdealTankMultiPort
    ...
    model TriplePortTank
      ...
      /*Fuel*/
      //Fuel model will be referenced From AZEAT_Validation
      replaceable package Medium = AZEAT_Validation.Fuel.Hydrogen constrainedby AZEAT_Validation.Fuel.Templates.SimpleFuel "Transported medium" annotation(
         choicesAllMatching);
  ...
  model Tank "Tank model for gas storage"
    ...
    partial model Tank_base
      ...
      connector FluidPort "Fluid port for low fidelity models in microgrid components"
        replaceable package Medium = AZEAT_Validation.Fuel.Hydrogen constrainedby AZEAT_ValidationFuel.Templates.SimpleFuel "Transported medium";
        Modelica.Units.SI.Pressure p "Pressure";
        flow Modelica.Units.SI.MassFlowRate m_flow "Mass flow rate";
        annotation(
          Icon(coordinateSystem(preserveAspectRatio = false), graphics = {Ellipse(extent = {{-100, 100}, {100, -100}}, lineColor = {28, 108, 200}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid)}),
      ...
      replaceable package Medium = AZEAT_Validation.Fuel.compressedhydrogen "Transported Medium" annotation(
        choicesAllMatching);
    ...
    //EndRepositroy
    extends Tank_base(E_store = energyDensity_max*V, redeclare replaceable package Medium = AZEAT_Validation.Fuel.compressedhydrogen constrainedby AZEAT_Validation.Fuel.Templates.SimpleFuel);
    //Local Parameters
...
// Connectors
  ...
  model FuelCellEfficiency
    partial model Base
      ...
      replaceable package Medium = AZEAT_Validation.Fuel.Hydrogen constrainedby AZEAT_Validation.Fuel.Hydrogen "Transported medium" annotation(
         choicesAllMatching);
...
// Summary
  ...
  parameter Real battery_Eff_Charge = 0.8;
  parameter Boolean battery_Set_SOC_Final_Start = true;
  parameter Real battery_Capacity = 15.48e+8;
  parameter Real battery_P_max = 0.1;
  ...
  //750, 100
  parameter Boolean mCtrl_user_defined_Bounds_diesel = true;
  parameter Real mCtrl_user_defined_upper_bound_diesel = 750;
  parameter Real mCtrl_user_defined_lower_bound_diesel = 200;
  ...
  parameter Real BSFC_Curve[:, 2] = [145.9375, 219.7885196; 153.125, 216.9939577; 164.0625, 213.0664653; 188.75, 206.0422961; 211.25, 201.2084592; 232.1875, 196.978852; 251.875, 194.0332326; 269.6875, 192.3716012; 282.5, 191.6918429; 302.8125, 192.5226586; 318.125, 192.8247734; 340.625, 193.9577039; 360, 195.3172205; 380.3125, 196.9033233; 409.6875, 199.244713; 445.625, 202.1903323; 468.4375, 204.0030211; 495.625, 206.9486405; 529.6875, 211.6314199; 555.9375, 215.4833837; 564.375, 216.8429003];
  ...
  // [0, 0.00; 10, 19.93; 20, 37.94; 30, 55.30; 40, 72.24; 50, 88.89; 60, 105.29; 70, 121.50; 80, 137.55; 90, 153.45; 100, 169.23]
  // [0, 0.00; 10, 24.41; 20, 46.47; 30, 67.73; 40, 88.48; 50, 108.86; 60, 128.96; 70, 148.81; 80, 168.47; 90, 187.95; 100, 207.27]
  // [0, 0.00; 10, 28.02; 20, 53.69; 30, 77.54; 40, 100.08; 50, 121.85; 75, 176.32; 85, 199.50; 100, 237.42] Prince Madog // this bsfc curve is from translated  engine  specification
  // [0, 0.00; 10, 46.08; 20, 88.29; 30, 127.50; 40, 164.57; 50, 200.37; 75, 289.94; 85, 328.07; 100, 390.41] Prince  Madog // this BSFC curve  is scaled
  parameter Real Engine_Fuel_Consumption_Look_Up_Table_Diesle[:, 2] =[0, 0.00; 10, 18.27; 20, 34.78; 30, 50.69; 40, 66.22; 50, 81.47; 60, 96.51; 70, 111.37; 80, 126.08; 90, 140.65; 100, 155.12];
  ...
  // Part Battery
  AZEAT_Validation.BatteryDualControl battery1(eff_charge = battery_Eff_Charge, set_SOC_final_start_ = battery_Set_SOC_Final_Start, SOC_max = mCtrl_SOC_max, SOC_min = mCtrl_SOC_min, capacity = battery_Capacity, P_max = battery_P_max, SOC_start = battery_SOC_start) annotation(
    Placement(transformation(origin = {10, 70}, extent = {{-10, -10}, {10, 10}})));
  // Part Electrical Untitlity
  AZEAT_Validation.Converter_ACDC converter_ACDC(efficiency = 1, V_ref_DC = 1, power_nominal = P_nominal, P_max = 10*P_nominal) "Inverter" annotation(
    Placement(transformation(origin = {108, 54}, extent = {{-10, -10}, {10, 10}})));
  AZEAT_Validation.ElectricalGrid electricalGrid(power_nominal = P_nominal, V_ref = 1000) annotation(
    Placement(transformation(origin = {156, 52}, extent = {{-10, -10}, {10, 10}})));
  AZEAT_Validation.ElectricLoad electricLoad(use_input = true, load_in = 1000) annotation(
    Placement(transformation(origin = {71, 29}, extent = {{11, -11}, {-11, 11}})));
  AZEAT_Validation.Transformer transformer(efficiency = 1, power_nominal = P_nominal, P_max = 10*P_nominal) annotation(
  ...
  // Part Mechanical
  AZEAT_Validation.Generator generator1(
  ...
  AZEAT_Validation.Generator generator2(power_vs_fuelconsum = Engine_Fuel_Consumption_Look_Up_Table_Diesle, V_flow_frat = generator_V_flow_Frat, V_flow_fidle = generator_V_flow_Fidle, use_const_eff = generator_Use_Const_Off, eta_el_const = generator_Eta_El_Const, fuelConsumptionIsVolumetric = true, m_flow_fidle = generator_M_Flow_Fidle, m_flow_frat = generator_M_Flow_FRat, P_rat = generator_P_rat, FLHV = generator_FLHV, Frho = generator_Frho, Frho_liq = generator_Frho_liq, FcarbonContent = generator_FcarbonContent, FMolarMass = generator_MolarMass, use_fuel_eff_table = generator_use_fuel_eff_table) annotation(
    Placement(transformation(origin = {-10, -25}, extent = {{10, -9}, {-10, 9}})));
  AZEAT_Validation.Generator generator3(power_vs_fuelconsum = Engine_Fuel_Consumption_Look_Up_Table_Diesle, V_flow_frat = generator_V_flow_Frat, V_flow_fidle = generator_V_flow_Fidle, use_const_eff = generator_Use_Const_Off, eta_el_const = generator_Eta_El_Const, fuelConsumptionIsVolumetric = true, m_flow_fidle = generator_M_Flow_Fidle, m_flow_frat = generator_M_Flow_FRat, P_rat = generator_P_rat, FLHV = generator_FLHV, Frho = generator_Frho, Frho_liq = generator_Frho_liq, FcarbonContent = generator_FcarbonContent, FMolarMass = generator_MolarMass, use_fuel_eff_table = generator_use_fuel_eff_table) annotation(
    Placement(transformation(origin = {-10, -56}, extent = {{10, -10}, {-10, 10}})));
  AZEAT_Validation.Generator generator_af_1(power_vs_fuelconsum = Engine_Fuel_Consumption_Look_Up_Table_AltFuel, V_flow_frat = generator_alt_V_flow_Frat, V_flow_fidle = generator_alt_V_flow_Fidle, use_const_eff = generator_Use_Const_Off, use_fuel_eff_table = generator_alt_fuel_eff_table, eta_el_const = generator_Eta_El_Const, fuelConsumptionIsVolumetric = true, m_flow_fidle = generator_alt_M_Flow_Fidle, m_flow_frat = generator_alt_M_Flow_FRat, P_rat = generator_alt_P_rat, FLHV = generator_alt_FLHV, Frho = generator_alt_Frho, Frho_liq = generator_alt_Frho_liq, FcarbonContent = generator_alt_FcarbonContent, FMolarMass = generator_alt_MolarMass) annotation(
    Placement(transformation(origin = {-10, -90}, extent = {{10, -10}, {-10, 10}}, rotation = -0)));
  AZEAT_Validation.Generator generator_af_2(power_vs_fuelconsum = Engine_Fuel_Consumption_Look_Up_Table_AltFuel, V_flow_frat = generator_alt_V_flow_Frat, V_flow_fidle = generator_alt_V_flow_Fidle, use_const_eff = generator_Use_Const_Off, eta_el_const = generator_Eta_El_Const, use_fuel_eff_table = generator_alt_fuel_eff_table, fuelConsumptionIsVolumetric = true, m_flow_fidle = generator_alt_M_Flow_Fidle, m_flow_frat = generator_alt_M_Flow_FRat, P_rat = generator_alt_P_rat, FLHV = generator_alt_FLHV, Frho = generator_alt_Frho, Frho_liq = generator_alt_Frho_liq, FcarbonContent = generator_alt_FcarbonContent, FMolarMass = generator_alt_MolarMass) annotation(
    Placement(transformation(origin = {-10, -130}, extent = {{10, -10}, {-10, 10}})));
  AZEAT_Validation.Generator generator_af_3(power_vs_fuelconsum = Engine_Fuel_Consumption_Look_Up_Table_AltFuel, V_flow_frat = generator_alt_V_flow_Frat, V_flow_fidle = generator_alt_V_flow_Fidle, use_const_eff = generator_Use_Const_Off, eta_el_const = generator_Eta_El_Const, use_fuel_eff_table = generator_alt_fuel_eff_table, fuelConsumptionIsVolumetric = true, m_flow_fidle = generator_alt_M_Flow_Fidle, m_flow_frat = generator_alt_M_Flow_FRat, P_rat = generator_alt_P_rat, FLHV = generator_alt_FLHV, Frho = generator_alt_Frho, Frho_liq = generator_alt_Frho_liq, FcarbonContent = generator_alt_FcarbonContent, FMolarMass = generator_alt_MolarMass) annotation(
    Placement(transformation(origin = {-10, -170}, extent = {{10, -10}, {-10, 10}}, rotation = -0)));
  AZEAT_Validation.IdealTankMultiPort idealTankMultiPort(m_content_max = idealTank_TankcontentDiesel, m_content_start = idealTank_TankcontentDieselStart, redeclare replaceable package Medium = AZEAT_Validation.Fuel.MethanolFuel) annotation(
    Placement(transformation(origin = {70, -28}, extent = {{-10, -10}, {10, 10}})));
  AZEAT_Validation.IdealTankMultiPort alt_fuel_tank(m_content_max = idealTank_TankcontentAltFuel, m_content_start = idealTank_TankcontentAltFuelStart, redeclare replaceable package Medium = AZEAT_Validation.Fuel.MethanolFuel) annotation(
    Placement(transformation(origin = {70, -70}, extent = {{-10, -10}, {10, 10}})));
  // Part Renewable
  ...
  Tank tank2(redeclare replaceable package Medium = AZEAT_Validation.Fuel.compressedhydrogen, SOC_start = H2SOCstart, V = H2Tankstorage, T(displayUnit = "K"), use_pressure_constraint = true) annotation(
    Placement(transformation(origin = {70, 142}, extent = {{-10, -10}, {10, 10}})));
  Tank tank3(redeclare replaceable package Medium = AZEAT_Validation.Fuel.compressedhydrogen, SOC_start = H2SOCstart, V = H2Tankstorage, T(displayUnit = "K"), use_pressure_constraint = true) annotation(
  ...
  // Part Controller
  AZEAT_Validation.MasterControllerSingleBattery masterControllerSingleBattery(n = N_gen, alt_n = N_alt_gen, acutal_diesel_n = N_diesel_gen_onboard, actual_altFuel_n = N_alt_fuel_gen_onboard, SOC_tank_sec = mCtrl_SOC_tank_Sec, SOC_min_sec = mCtrl_SOC_min_Sec, P_charging_max = battery1.P_max, P_max = {generator1.P_rat, generator2.P_rat, generator3.P_rat}, P_max_alt = {generator_af_1.P_rat, generator_af_2.P_rat, generator_af_3.P_rat}, DieselControlforBattery = mCtrl_DieselControlforBattery, SOC_max = mCtrl_SOC_max, SOC_min = mCtrl_SOC_min, diesel_gen_charge_rate = generator_P_charging, alt_gen_charge_rate = generator_alt_P_charging, battery_rated_gen = battery_P_max, diesel_gen_idling_rate = generator_P_idle, alt_gen_idling_rate = generator_alt_P_idle, P_fuelCellMax_ref = fuelCell3.P_rat, P_rate_combined_ICE_gen = (generator_P_rat*N_diesel_gen_onboard + generator_alt_P_rat*N_alt_fuel_gen_onboard), P_max_onboard = generator_actual_max_total, P_alt_max_onboard = alt_generatoractual_max_total, ParameterAssignement = mCtrl_PiroritySchemeAssignement, IcePriorityAssignement = mCtrl_IcePriorityAssignement, IceChargingPriorityAssignment = mCtrl_IceChargingPriorityAssignement) annotation(
    Placement(transformation(origin = {-60, 47}, extent = {{-18, -17}, {18, 17}})));
  Modelica.Blocks.Tables.CombiTable1Ds combiTable1Ds(tableOnFile = true, tableName = "tab1", fileName = "C:/Users/NMS08/Desktop/ReCreatedModel/AZETValidationCase/2024-04_FC_Cardinal_buoy_maintenance.txt", verboseRead = true, columns = 2:2) annotation(
    Placement(transformation(origin = {192, 2}, extent = {{10, -10}, {-10, 10}})));
  ...
  AZEAT_Validation.OptimalFuelConsumptionBoundsCalculator_ExternalTable altFuel_bsfc_table(BSFC_Curve_Default = BSFC_Curve, manually_define_bounds = mCtrl_user_defined_Bounds_alt_fuel, manual_defined_upperBound = mCtrl_user_defined_upper_bound_alt_fuel, manual_defined_lowerBound = mCtrl_user_defined_lower_bound_alt_fuel) annotation(
    Placement(transformation(origin = {-230, 132}, extent = {{10, 10}, {-10, -10}})));
  AZEAT_Validation.OptimalFuelConsumptionBoundsCalculator_ExternalTable diesel_bsfc_table(BSFC_Curve_Default = BSFC_Curve_AltFuel, manually_define_bounds = mCtrl_user_defined_Bounds_diesel, manual_defined_upperBound = mCtrl_user_defined_upper_bound_diesel, manual_defined_lowerBound = mCtrl_user_defined_lower_bound_diesel) annotation(
...
equation
  ...
  connect(time_expre2.y, combiTable1Ds.u) annotation(
    Line(points = {{259, 0}, {231, 0}, {231, 2}, {204, 2}}, color = {0, 0, 127}));
  connect(combiTable1Ds.y[1], gain1.u) annotation(
    Line(points = {{181, 2}, {158.5, 2}, {158.5, 0}, {136, 0}}, color = {0, 0, 127}));
